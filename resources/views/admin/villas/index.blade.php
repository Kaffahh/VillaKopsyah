...
@section('content')
        <div class="card shadow-sm">
            <div class="card-header py-3"><h4 class="mb-0">Daftar Villa</h4></div>
            <div class="card-body">
                @if (session('status'))
                ...
                <div class="mb-3">
                    <a href="{{ route('admin.villas.create') }}" class="btn btn-primary btn-sm">Tambah Villa</a>
                </div>
                        ...
                        <tbody>
                            @forelse ($villas as $villa)
                                <tr>
                                    ...
                                    <td>Rp
                                        {{ number_format(optional($villa->prices->last())->price_per_night, 0, ',', '.') }}
                                    </td>
                                    <td>{{ optional($villa->capacities->last())->capacity ?? 'Tidak tersedia' }}</td>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $villa->status === 'available' ? 'bg-success' : 'bg-danger' }}">
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('admin.villas.destroy', $villa->id) }}" method="POST"
                                            ...
                                            <button type="submit" class="btn btn-danger btn-sm"
                                                onclick="return confirm('Apakah Anda yakin ingin menghapus villa ini?')">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="11" class="text-center">Tidak ada data villa tersedia.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
